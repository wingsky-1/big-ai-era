...
## 约束：RefCounted 零 Node（headless 可单测）；禁 import L3/L4；数值零硬编码
## （tech-tree-spec D.2 键名真源；tree_research_* 留 #138 分区）；五态=enum 单向
## 链恰一档推进（非法拒绝+push_error）；三通路=周推进（rng.insight+pity，
## pity 读 rng.json rnd_insight_pity_max）/竞对论文外溢（注入表源 #144 真接线）/
## 交叉进度（他域 lit 联动，tree_cross_nodes 表驱动）；fog_changed 信号见
## architecture-100 §5.2。

signal fog_changed(payload: Dictionary)

## 迷雾五态（tech-tree-spec A.1；视图字符串经 _state_key 映射）
enum FogState {
	HIDDEN,
	RUMORED,
	VISIBLE,
...
## tech-tree-spec D.2 键名（#137 前缀分区：tree_fog_*/tree_insight_* 本单，
## tree_research_* 归 #138 同文件增量，git 自动合并）
...
## ??? 掩码行/传闻行文案键（C.1 tree_fog_hidden/tree_fog_rumored；L3 渲染）
...
var _cross_reveal_max: int = 0  # tree_reveal_cross：每次联动最多推进节点数（1）
var _domain_node_count: int = 0  # tree_domain_nodes：每域节点数

...
func test_modal_sizing_clamps_within_narrow_viewport() -> void:
	...
	assert_eq(clamped.x, 432.0, "tech_tree 宽应精确收敛到 480-48=432")
...
func test_modal_sizing_floor_protects_tiny_viewport() -> void:
	# 极小视口（如 headless 64x64 测试窗）：不得塌缩到 0，须守住下限
	var floored: Vector2 = ModalSizing.resolve_min_size(Vector2(64, 64), Vector2(520, 440))
	assert_eq(floored, ModalSizing.MIN_MODAL_SIZE, "极小视口下弹层收敛到 MIN_MODAL_SIZE 下限")


func test_modal_sizing_apply_and_refresh_roundtrip() -> void:
	# 用裸 PanelContainer 测纯函数行为：%UniqueName 依赖场景注册，脚本化节点无此表
	# 显式设 root 尺寸：GUT headless 默认窗口 64x64，不设则断言被窗口尺寸污染
	...
	assert_eq(dlg.custom_minimum_size.x, 432.0, "窄视口下 apply 应精确收敛到 432")
...
func test_all_modal_design_sizes_fit_landscape_base() -> void:
	...
	# 键为蛇形文件名，与 apply() 的 resource_path 派生键一致（评审 P0 联动）
	var expected := {

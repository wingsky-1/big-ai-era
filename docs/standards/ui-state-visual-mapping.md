...
| V1-12（#104 PR-C 接线） | decision_pending(z2) / notice_toast(z3) | decision_pending（**#104 PR-C 前为死信号：11 个契约信号中唯一零 emit 点**，`settle_week` 无 policy 时自动选 0 号选项 → 卡永不显示；现由事件层出卡 → L2 广播 → presenter 推 z2） | z2 决策卡：标题/叙事段/2–3 选项（各带效果预览句 decision_option_hint_*）常显直到选择；通知级=z3 toast 短显+归入当周周报行；通知层零红点零角标 | 弹出动画（同帧按信号到达序串行，决策卡先于周报）；选项按下反馈；选择后卡收起 | 遮罩点击不关闭（强迫处理）；未解锁选项不出现（非置灰） | 待回填 | 卡纵向滚动满宽；选项键纵排≥48px | 12 事件池扩容决策卡/通知 toast（#41，通知层零红点零角标） |
...
| V1-26（#104 PR-C 已实施） | 手动存档 idle / saved | 无信号（玩家主动触发）；存档结果经 `toast_queued`（texts.json 既有键 `sys_save_hint`） | 暂停菜单常显「存档」键（与继续/重开/设置并列） | 点「存档」→ 契约命令 `request_save("manual")` → SaveSystem 原子写（tmp→校验→rename）→ toast「进度已保存」 | 无禁用态（随时可存；写失败由 SaveSystem 保留磁盘原档） | 待回填（建议 `ui_manual_save`） | 暂停菜单纵排满宽 ≥48px（既有布局） | #104 PR-C（此前 `request_save` 在 `src/ui/**` 零调用方，三保险之一无玩家触发）；[T] `test_pause_menu_save_button_writes_save` |
| V1-25（#104 PR-B 已实施） | training_board：可启动 / 不可启动（含原因）/ training | `resources_changed`（既有）+ `week_settled`（既有）→ L2 `get_training_view()` | 工作区「训练」轻键常显；训练进行中行常显「基座名 · 剩余 N 周」+ 进度条（未训练时进度条隐藏）；z1 训练板每行常显：基座名 + 「工期/成本/质量/所需算力档/上桌上限/每周卡时」+ 状态文案 | 启动键按下 → 契约命令 `start_training` → 行状态翻新为「训练中」+ 工作区训练行即时出现 | 不可启动时启动键置灰 + 行内原因（已有训练进行中 / 无在岗研究员 / 算力档不足 / 资金不足 / 上桌人数超限 / 本周卡时不足） | 待回填（建议 `ui_training_board_state`） | 工作区轻键与训练行纵排满宽 ≥48px；弹层满宽纵排、行内换行不滚动 | #104 PR-B（此前 `start_training` 在 `src/ui/**` 零调用方、工作区完全不渲染 training）；需求 §3.3/§8.3 + DR-031/B2（min_staff 死键不展示）；[T] `test_training_rows_match_bases_json` / `test_blocked_reason_single_source` / `test_workspace_training_visible` |
| V1-24（#104 已实施） | task_board：可接 / 不可接（含原因）/ active / queued | `task_state_changed`（既有）+ `resources_changed`（既有）+ `week_settled`（既有）→ L2 `get_task_board_view()` | 工作区「接单」轻键常显（工作区永不折叠，DR-009/015）；z1 任务板每行常显：任务名 + 「工期 N 周 · 收入 ¥X · 声望 Y · 成本 ¥Z」+ 状态文案；进行中行常显「名称 · 剩余 N 周」+ 进度条（周粒度） | 接单键按下 → 契约命令 `enqueue_task` → 行状态翻新为「进行中/排队中」+ 工作区任务行即时刷新 | 不可接时接单键置灰 + 行内原因文案（资金不足 / 未满足解锁条件 / 已在进行·排队中）——**拒绝分支可见**（ui-feedback-checklist §1 三态） | 待回填（建议 `ui_task_board_state`） | 工作区轻键纵排满宽 ≥48px；弹层满宽纵排、行内自动换行不横向滚动 | #104 P0 玩家入口（此前 `enqueue_task` 在 `src/ui/**` 零调用方）；需求 §2.3 RT-01 / §1.3；数据面 `ui_display.json.task_board`；[T] `test_task_rows_match_tasks_json` / `test_blocked_reason_single_source` / `test_workspace_active_task_visible` |

...
- 真源：`docs/standards/seat-io-contracts.md`。
- 输入把控（拒领权）：未过检 issue（缺验收点/未过 lint+内容审）**有权拒领**并退回补规格——禁带病接手；含糊处先评论问清再动工。
- 输出把控：完工 DoD=verify 全绿+**验收点自证表**（逐条引用证据：GUT 用例名+该用例 passing 输出行/截图路径，禁自然语言复述）；PR body 必须引用 issue（无 issue 不合码）。
